...
header = rdd.first()
df = rdd.filter(lambda x: x != header).map(lambda x: x.split(',')).map(lambda x: (x[0], x[1])).groupByKey().filter(lambda x: len(x[1]) >= 7).cache()
pairs = []
df2 = df.collectAsMap()
...
while sorted_edges:
    ...
    sorted_edges = sc.parallelize(updated_matrix.keys(),1).flatMap(lambda x: iterate_vertexes(x, updated_matrix)).map(
